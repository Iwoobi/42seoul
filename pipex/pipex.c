...
// /*   Updated: 2022/06/03 19:27:10 by youhan           ###   ########.fr       */
// /*                                                                            */
// /* ************************************************************************** */
...
void	push_infile_data(int w_fd, int r_fd)
{
	char	*tmp;
	...
	while (tmp != NULL)
	{
		write(w_fd, tmp, ft_strlen(tmp));
		free(tmp);
		tmp = get_next_line(r_fd);
	}
}

void	make_infile_pipe(t_list *data)
{
	dup2(data->in_pipe[1], 1);
	close(data->in_pipe[0]);
	all_close_read_pipe_fd(data);
	all_close_write_pipe_fd(data);
	// push_infile_data(data->fd.w_fd, data->fd.r_fd);
...
void	contral_first_process(t_list *data)
{
	...
	all_close_read_pipe_fd(data);
	all_close_write_pipe_fd(data);
	// make_infile_pipe(data);
		exist_cat(data, &dir_cmd);
}
void	contral_first_cmd_process(t_list *data, char **env)
{
	...
	close(data->in_pipe[1]);
	dup2(data->in_pipe[0], 0);
	close_write_pipe_fd(data, 0);
	all_close_read_pipe_fd(data);
...
void	contral_last_process(t_list *data, char **env)
{
	...
	close(data->in_pipe[1]);
	all_close_write_pipe_fd(data);
	close_read_pipe_fd(data, data->pipe_num);
	...
	close(data->p_fd[data->pipe_num - 1][0]);
	close(data->fd.w_fd);
	free(dir_cmd);
}
...
void	contral_mid_process(t_list *data,int process, char **env)
{
	...
	close(data->p_fd[process - 1][0]);
	close(data->p_fd[process][1]);
	free(dir_cmd);
}
...
void	malloc_status(t_list *data)
{
	int	i;

	data->status = (int *)malloc(sizeof(int) * data->pipe_num);
	if (data->status == NULL)
		exit(1);
	i = 0;
	while (i < data->pipe_num)
...
void	check_error_child(t_list *data)
{
	int	i;
...
// void	push_infile_data(int fd, char **infile_data)
// {
// 	char	*tmp;
...
// 	while (tmp != NULL)
// 	{
// 		join_infile_data(infile_data, &tmp);
// 		free(tmp);
// 		tmp = get_next_line(fd);
// 	}
// }
...
// 	if (pipe(data->in_pipe) == -1)
// 		error_code(2);
// 	push_infile_data(data->fd.r_fd, &infile_data);
// 	dup2(data->in_pipe[1], 1);
// 	dup2(data->in_pipe[0], 0);
// 	write(data->in_pipe[1], infile_data, ft_strlen(infile_data));
// 	close(data->in_pipe[1]);
// 	free(infile_data);
// }
// void	contral_first_process(t_list *data, char **env)
// {
// 	char	*dir_cmd;
...
// 			contral_first_process(data, env);
// 		else if (process_num(*data, i) == 1)
