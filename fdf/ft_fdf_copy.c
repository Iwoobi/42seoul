...
/*   Updated: 2022/05/19 13:29:45 by youhan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	draw_line_color_1(t_mlx *my_mlx, t_line *line, t_index i)
{
	if (line->val_z[1] > my_mlx->data->color)
		return (22);
	else if (line->val_z[0] > my_mlx->data->color)
		return (21);
	else
		return (11);
}
int	draw_line_color_2(t_mlx *my_mlx, t_line *line, t_index i)
{
	if (line->val_z[0] > my_mlx->data->color)
		return (-22);
	else if (line->val_z[1] > my_mlx->data->color)
		return (-12);
	else
		return (-11);
}

void	draw_line_color(t_mlx *my_mlx, t_line *line, t_index i)
{
	int	mod;

	if(line->val_z[0] > line->val_z[1])
		mod = draw_line_color_1(my_mlx, line, i);
	else
		mod = draw_line_color_2(my_mlx, line, i);
	line->mod = 0;
	if (mod == 22 || mod == -22)
		return ;
	line->mod = -1;
	...
	if (mod < 0)
	{
		line->x_dot = ((my_mlx->data->color - line->val_z[1]) * line->w / (line->val_z[1] - line->val_z[0])) + line->x;
		line->y_dot = ((my_mlx->data->color - line->val_z[1]) * line->h / (line->val_z[1] - line->val_z[0])) + line->y;
		return ;
	}
	else
	{
		line->x_dot = ((my_mlx->data->color - line->val_z[0]) * line->w / (line->val_z[1] - line->val_z[0])) + line->x;
		line->y_dot = ((my_mlx->data->color - line->val_z[0]) * line->h / (line->val_z[1] - line->val_z[0])) + line->y;
	}
	line->mod = 2;
}
int	draw_line_color_fin(t_line *line)
{
	if (line->mod == 0)
		return (0xFF0000);
	if (line->mod == -1)
		return (0xFFFFFF);
	if (line->mod == 1 && line->x_dot < line->x)
		return (0xFF0000);
	if (line->mod == 1 && line->x_dot >= line->x)
		return (0xFFFFFF);
	if (line->mod == 2 && line->x_dot < line->x)
		return (0xFF0000);
	if (line->mod == 2 && line->x_dot >= line->x)
		return (0xFFFFFF);
	return (0x00FF00);
}
int	draw_line_color_fin1(t_line *line)
{
	if (line->mod == 0)
		return (0xFF0000);
	if (line->mod == -1)
		return (0xFFFFFF);
	if (line->mod == 1 && line->y_dot > line->y)
		return (0xFF0000);
	if (line->mod == 1 && line->y_dot <= line->y)
		return (0xFFFFFF);
	if (line->mod == 2 && line->y_dot > line->y)
		return (0xFF0000);
	if (line->mod == 2 && line->y_dot <= line->y)
		return (0xFFFFFF);
	return (0x00FF00);
	...
	t_line *line, double *start, double *fin)
{
	...
	while (line->x != (int)(fin[0]))
	{
		...
		if (line->x < 1000 && line->y < 1000 && line->x >= 0 && line->y >= 0)
			my_mlx->img.data[line->y * 1000 + line->x] = draw_line_color_fin(line);
		line->x++;
		if (start[0] - fin[0] > 0)
			line->x -= 2;
	}	
	...
	t_line *line, double *start, double *fin)
{
	...
	while (line->y != (int)fin[1])
	{
		...
		if (line->x < 1000 && line->y < 1000 && line->x >= 0 && line->y >= 0)
			my_mlx->img.data[line->y * 1000 + line->x] = draw_line_color_fin1(line);
		line->y++;
		if (start[1] - fin[1] > 0)
			line->y -= 2;
	}
...
void	draw_line(double *start, double *fin, t_mlx *my_mlx, t_index i)
{
	t_line	line;
	...
	line.val_z[0] = my_mlx->data->arr_copy[i.i][i.j][2];
	if (i.mod == 1)
		line.val_z[1] = my_mlx->data->arr_copy[i.i][i.j + 1][2];
	else
		line.val_z[1] = my_mlx->data->arr_copy[i.i + 1][i.j][2];
	draw_line_color(my_mlx, &line, i);
	if (line.w > line.h)
...
void	draw_lines(t_list *data, t_mlx *my_mlx)
{
	...
	while (i.i < data->row)
	{
		...
		while (i.j < data->col)
		{
			i.mod = 1;
			if (i.j != data->col - 1)
				draw_line((data->arr)[i.i][i.j], (data->arr)[i.i][i.j + 1], my_mlx, i);
			i.mod = 2;
			if (i.i != data->row - 1)
				draw_line((data->arr)[i.i][i.j], (data->arr)[i.i + 1][i.j], my_mlx, i);
			i.j++;
		}
		i.i++;
	}
...
void	ft_fdf_init(t_list *data)
{
	...
	data->mul = 1;
	data->color = (data->max_val + data->min_val) / 2;
